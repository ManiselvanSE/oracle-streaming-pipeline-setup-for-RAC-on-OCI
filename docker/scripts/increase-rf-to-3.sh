...
TOPICS=$(docker exec kafka2 kafka-topics --bootstrap-server $BOOTSTRAP --list 2>/dev/null | grep -E 'racdb|__orcl|__cflt' || true)
FIRST=1
...
docker exec kafka2 kafka-reassign-partitions \
...
docker exec kafka2 kafka-topics --bootstrap-server $BOOTSTRAP --describe --topic racdb.ORDERMGMT.MTX_TRANSACTION_ITEMS
rm -f "$REASSIGN_JSON"
